      ...
      <div id="outputText">
        ...
        <p>
          <strong>Speed</strong> {{ thrusters.speed + thrustersBooster.speed
            ...
            v-if="
              params.sourcesInUse.som && params.thrustersMaterialId != 'none'
            "
          >
            ({{ params.thrustersMaterialId }} thrusters)</span
          >; <strong>Maneuverability</strong> {{ frame.maneuverability }} (turn
          {{ turn }})<span
            v-if="
              driftEngine.engineRating == 'Special' ||
              driftEngine.engineRating > 0
            "
            >; <strong>Jump Distance</strong>
            {{ driftEngine.engineRating }}</span
          >
        </p>
        <p>
          <strong>AC</strong> {{ armorClass }}; <strong>TL</strong>
          {{ targetLock }}
        </p>
        <p>
          <strong>HP</strong> {{ hp }}; <strong>DT</strong>
          {{ damageThreshold }}; <strong>CT</strong> {{ criticalThreshold }}
        </p>
        ...
        <p v-if="params.shieldType == 'shields'">
          ...
          {{ params.shieldsByPosition.forward }}, port
          {{ params.shieldsByPosition.port }}, starboard
          {{ params.shieldsByPosition.starboard }}, aft
          {{ params.shieldsByPosition.aft }})
        </p>
        <!-- deflector shields -->
        <p v-else>
          <strong>Deflector Shield</strong> {{ deflectorShield.name }};
          <strong>DV</strong> {{ deflectorShield.defenseValue }}/&ndash;
        </p>
        <!-- reinforced bulkheads -->
        <p
          v-if="
            params.sourcesInUse.som && params.reinforcedBulkheadId != 'none'
          "
        >
        ...
        <p>
          ...
          <strong>Hyperdrive</strong> {{ driftEngine.name }};
          <strong>Systems </strong> <span v-html="systemsDescription"></span
          ><span v-if="hasSecurity"
            >; <strong>Security</strong> {{ securityDescription }}</span
          ><span v-if="expansionBaysDescription != 'None'"
            >; <strong>Expansion Bays</strong>
            {{ expansionBaysDescription }}</span
        ...
        <p>
          ...
          }}<span v-if="params.hasCrew"
            >; <strong>Complement</strong> {{ complement }}</span
          >
        </p>
        ...
        <p>
          <strong>Build Points</strong> cost {{ totalBpCost }}, max
          {{ tier.bpBudget }} <strong>Power Core Units</strong> non-essential
        ...
        <div v-if="params.hasCrew">
          ...
          <p v-for="roleId in selectOptionsCrew">
            <strong>{{ roleDescription[roleId] }}</strong>
            {{ crewDescriptions[roleId] }}
          </p>
          <p v-if="params.viId != 'none'">
            <strong>VI</strong> {{ viCrewDescription }}
          </p>
        </div>
